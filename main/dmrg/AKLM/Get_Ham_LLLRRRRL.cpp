...
void Get_Ham_LLLRRRRL(CRS &Ham_LLLRRRRL, Block_Operator &System, Block_Operator &Enviro, DMRG_Basis &Basis, DMRG_Basis_Stored &Basis_System, DMRG_Basis_Stored &Basis_Enviro, DMRG_Block_Information &Block, Model_1D_AKLM &Model) {
   ...
   for (int i = 0; i < dim_LL; i++) {
      Block_Ham.Ele_LL[i] = Basis_System.QN1_LL_LL_Stored[LL_site][i];
   }
   for (int i = 0; i < dim_RR; i++) {
      Block_Ham.Ele_RR[i] = Basis_Enviro.QN1_LL_LL_Stored[RR_site][i];
   }
   for (int i = 0; i < dim_onsite; i++) {
      Block_Ham.Ele_On[i] = Basis_System.QN1_LL_LL_Stored[   0   ][i];
   }
   
   Get_Ham_LLLR(System, Basis.LLLR, Block_Ham.Ele_LL, LL_site, Model, Block_Ham.LLLR);
   Get_Ham_RRRL(Enviro, Basis.RRRL, Block_Ham.Ele_RR, RR_site, Model, Block_Ham.RRRL);
   Get_Ham_LRRL(Basis.LRRL, Block_Ham.Ele_On, Model, Block_Ham.LRRL);
