...
void Renormalize(Block_Operator &System, Block_Operator &Enviro, DMRG_Basic_Information &Info, DMRG_Basis_Stored &Basis_System, DMRG_Basis_Stored &Basis_Enviro, Model_1D_AKLM_TVF &Model, DMRG_Param &Dmrg_Param, Diag_Param &Diag_Param) {
   ...
   DMRG_Construct_Superblock(Info, Basis_System, Basis_Enviro, Model.num_of_qn, Dmrg_Param.Initial_Guess, "qn2", Model.p_thread, Time);
   ...
   Expectation_Values(Info.GS, Info.Basis_LLLRRRRL, Info.Block_Compo, Basis_System, Basis_Enviro, Info.QN, Model, Dmrg_Param);
   
   Time.total = omp_get_wtime() - start;
   
   DMRG_Print_Status("Ele+Parity", Model.BC, Info, Dmrg_Param, Time);
